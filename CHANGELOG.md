...
### Features
- **工具箱**: 7 个可拖拽浮动面板 — 组合仓位+分红（百分比/股数双模式）、复利计算器、安全边际、仓位管理、分红日历、换股计算器、凯利仓位
- **凯利仓位**: 输入胜率和盈亏比，计算全凯利/半凯利/1/4 凯利三档仓位比例，含期望值、破产概率估算和实战建议
- **ROI 柱状图**: 拆分为资本回报（青色）+ 分红回报（粉色）堆叠柱状图，右侧显示合计值
- **数据源扩展**: Baostock（免费历史 K 线兜底）+ Tushare Pro（免费财务报表+融资融券）
- **行业自动回写**: 刷新快照时自动将行业数据填充到监控股票（仅填空，不覆盖）

### Bug Fixes
- 分红日历支持一年多次分红的公司，按去年分红次序预估，已发过的自动跳过
- 快照刷新 UNIQUE constraint 报错修复（`ignore_conflicts=True` + dict 去重）
- 首页股票卡片去掉整体点击跳转，仅底部按钮可跳转
- 隐藏"更新/帮助"按钮
