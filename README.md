...
- 工具箱：可拖拽浮动面板 — 组合仓位+分红（实时概览：组合市值/浮动盈亏/权重偏离/再平衡建议，股数模式可录入买入价计算成本）、复利计算器、安全边际、仓位管理、分红日历、换股计算器、凯利仓位。
...
- `GET /api/portfolio/` — 获取默认组合持仓
- `POST /api/portfolio/save/` — 保存组合（支持录入买入价）
- `GET /api/portfolio/summary/` — 组合实时概览：实时市值、浮动盈亏、权重偏离、再平衡建议（仅拉取组合内持仓实时价，低负载）

## 最近更新

- 逻辑 bug 修复（6 处）：卡片 ROI 补传股息率（与 tooltip/排序逻辑一致）；财报日历年报披露窗口起始年份修正为次年；组合总盈亏不再被无成本持仓虚增；历史股息率仅在真实分红明细缺失时才用实时值兜底；深度刷新现在能清除 hist_single/intraday 历史缓存（不再返回 7 天陈旧数据）；选股器深度筛选/排序改为对全量候选集计算 flags 后再分页（total 与全局排序正确）。
- 缓存层统一失效 + 刷新入口闭合：invalidate 同时删裸键与「_v2」键，闭合 macro/earnings/分红日历的「?force=1」主动刷新；前端分析/回测刷新按钮补 force 参数（修此前“假刷新”）；前端区分 cache_status 的 error/stale/empty 状态，仅 error 弹失败提示；移除主页“上次更新”时间戳。
- 打包安全：禁止把本机 db.sqlite3（私人持仓）复制进桌面 exe——package.json 排除 db/cache/.env 等额外资源，main.cjs 移除 seedDb 复制，prepare-package.cjs 打包前物理清场。
- 分红日历修复：最近已分红则跳过当期预估，过滤多年前的孤立除权日噪声，避免已分股票仍显示“将分红”和异常早期除权日。

- 组合管理实时概览：新增 `portfolio_service` 与 `GET /api/portfolio/summary/` 接口，Dashboard 浮动面板「组合仓位+分红」展示组合实时市值、浮动盈亏（红涨绿跌）、权重偏离条与再平衡建议；股数模式可录入买入价用于成本计算。仅拉取组合内持仓实时价，低负载。
...
- `backend/db.sqlite3` 含本地私有数据（组合持仓/私人配置），已在 `.gitignore` 中忽略，不会提交到 Git；分发桌面 exe 时请注意其中同样包含该数据库。
