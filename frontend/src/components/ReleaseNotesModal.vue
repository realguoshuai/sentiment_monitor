...
const updates = [
  {
    title: '深度分析 ROIC 指标修复',
    text: '当财报数据缺少现金/负债字段时，ROIC 不再全部显示为 0，自动用归母净资产兜底计算，资本配置标签和投资论点评分恢复正常。',
  },
  {
    title: '后端代码质量全面提升',
    text: '修复裸 except 吞掉 KeyboardInterrupt 的隐患；datetime 统一使用 timezone.now()；N+1 查询优化（情感走势接口从 63 次查询降到 3 次）；可变默认参数、位置参数耦合等问题一并清理。',
  },
  {
    title: '分红方案解析防御加固',
    text: '分红方案中送/转/派金额解析增加异常保护，上游数据格式异常时不再导致分红日历和盯盘日记报错。',
  },
  {
    title: '前端股价对比页面稳定性',
    text: '修复实时价格和涨跌幅在数据未到达时 .toFixed() 崩溃的问题，增加 optional chaining 防护。',
  },
]
